...
it("주간 뷰에서 이전으로 navigate시 7일 전 '2024-09-24' 날짜로 지정이 된다", () => {
  ...
  assertDate(result.current.currentDate, '2024-09-24');
});
...
it("월간 뷰에서 다음으로 navigate시 한 달 후 '2024-11-01' 날짜여야 한다", () => {
  ...
  assertDate(result.current.currentDate, '2024-11-01');
});
...
it("월간 뷰에서 이전으로 navigate시 한 달 전 '2024-09-01' 날짜여야 한다", () => {
  ...
  assertDate(result.current.currentDate, '2024-09-01');
});
...
it("currentDate가 '2024-01-01' 변경되면 1월 휴일 '신정'으로 업데이트되어야 한다", async () => {
  ...
  act(() => {
    result.current.setCurrentDate('2024-01-01');
  });

  expect(result.current.holidays).toEqual([{ date: '2024-01-01', name: '신정' }]);
});
